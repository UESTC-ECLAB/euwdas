...
class RegisterForm(Form):
    username = TextField('Username', [
        validators.Required(message = u"必须填写用户名"),
        validators.Length(min = 3, message = u"用户名长度过短（3-12个字符）"),
        validators.Length(max = 12, message = u"用户名长度过长（3-12个字符）"),
        validators.Regexp("^[a-zA-Z][a-zA-Z0-9_]*$", message = u"用户名格式错误（英文字母开头，数字，下划线构成）"),
    ])

    email = TextField('Email', [
        validators.Required(message = u"必须填写Email"),
        validators.Length(min = 4, message = u"Email长度有误"),
        validators.Email(message = u"Email地址无效"),
    ])

    password = TextField('Password', [
        validators.Required(message = u"必须填写密码"),
        validators.Length(min = 6, message = u"密码长度过短（6-64个字符）"),
        validators.Length(max = 64, message = u"密码长度过长（6-64个字符）"),
        validators.EqualTo('password_confirm', message=u'两次输入密码不一致'),
    ])
...
class SettingForm(Form):
    ...
    website = TextField('Website', [
        validators.Optional(),
        validators.URL(message = "请填写合法的URL地址（如：http://eclab.xyz）")
    ])
